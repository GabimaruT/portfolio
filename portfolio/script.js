/* ═══════════════════════════════════════════════════════════
   GabimaruT Portfolio — Cinematic Scroll Engine & Slider Logic
   ═══════════════════════════════════════════════════════════ */
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  // ── Infinite Slider Setup & Logic ────────────────────────
  function setupSightSlider() {
    if (!sightsTrack || originalCards.length === 0) return;
  ...
  function updateSightSlider() {
    ...
    const computedStyle = window.getComputedStyle(sightsTrack);
    const gap = parseFloat(computedStyle.columnGap || computedStyle.gap || "0");

    const shiftPx = -(cardWidth + gap) * activeSight;
  ...
  function update() {
    ...
    if (Math.abs(smoothScroll - targetScroll) < 0.01) smoothScroll = targetScroll;
    ...
    // Custom properties update with explicit precision
    const effMX = reduceMotion.matches ? 0 : mouseX;
    const effMY = reduceMotion.matches ? 0 : mouseY;

    root.style.setProperty("--mx", effMX.toFixed(4));
    root.style.setProperty("--my", effMY.toFixed(4));

    root.style.setProperty("--back-opacity", (1 - frame2.active * 0.06).toFixed(4));
    root.style.setProperty("--back-x", `${(effMX * -12).toFixed(4)}px`);
    root.style.setProperty("--back-y", `${(effMY * -4).toFixed(4)}px`);
    ...
    root.style.setProperty("--bridge-x", `calc(-50% + ${(effMX * 18).toFixed(4)}px)`);
    root.style.setProperty("--bridge-y", `${(effMY * 8 + sharedHeroY - frame2.exit * 760).toFixed(4)}px`);
    ...
    root.style.setProperty("--split-left-x", `calc(-50% + ${(-splitDrift * 46).toFixed(4)}vw + ${(effMX * 22).toFixed(4)}px)`);
    root.style.setProperty("--split-left-y", `${(effMY * 10 + sharedHeroY - splitDrift * 180).toFixed(4)}px`);
    root.style.setProperty("--split-left-scale", (1 + sharedHeroScale + frame2.enter * 0.74).toFixed(4));
    root.style.setProperty("--split-right-x", `calc(-50% + ${(splitDrift * 46).toFixed(4)}vw + ${(effMX * 22).toFixed(4)}px)`);
    root.style.setProperty("--split-right-y", `${(effMY * 10 + sharedHeroY - splitDrift * 180).toFixed(4)}px`);
    ...
    root.style.setProperty("--frame2-x", `calc(-50% + ${(effMX * 10).toFixed(4)}px)`);
    root.style.setProperty("--frame2-y", `calc(-50% + ${(effMY * 8 - frame2.exit * 150).toFixed(4)}px)`);
    ...
    // Schedule next frame if values are still changing
    if (
      Math.abs(smoothScroll - targetScroll) > 0.01 ||
      Math.abs(mouseX - targetMouseX) > 0.0005 ||
      Math.abs(mouseY - targetMouseY) > 0.0005
    ) {
      requestTick();
    }
  }
